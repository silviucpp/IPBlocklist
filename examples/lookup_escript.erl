...
lookup(Feeds, IpBin) ->
    case parse_ip(IpBin) of
        {ok, v4, Target} ->
            do_lookup(Feeds, Target, v4);
        {ok, v6, Target} ->
            do_lookup(Feeds, Target, v6);
        Error ->
            Error
    end.
...
read_strings(Rest, 0, _, Acc) ->
    {Acc, Rest};
read_strings(<<Len:8, Name:Len/binary, Rest/binary>>,
             N, I, Acc) ->
...
read_feeds(Bin, N, Flags, Cats, Acc) ->
    <<NLen:8, Name:NLen/binary,
      BS:8, CO:8, FM:32/little, CM:8,
      RC:32/little, Rest/binary>> = Bin,
    {V4S, V4E, V6S, V6E, Rest2} =
        read_ranges(Rest, RC, 0, [], [], [], []),
    Feed = #{
        name => Name,
        base_score => BS,
        confidence => CO,
...
read_ranges(Bin, N, Cur, V4S, V4E, V6S, V6E) ->
    ...
    case End =< ?IPV4_MAX of
        true ->
            read_ranges(R2, N - 1, Start,
                [Start | V4S], [End | V4E], V6S, V6E);
        _ ->
            read_ranges(R2, N - 1, Start,
                V4S, V4E, [Start | V6S], [End | V6E])
    end.
...
do_lookup(Feeds, Target, Version) ->
...
parse_ip(IpBin) ->
    case inet:parse_address(binary_to_list(IpBin)) of
        {ok, {A, B, C, D}} ->
            {ok, v4,
                (A bsl 24) bor (B bsl 16)
                bor (C bsl 8) bor D};
        {ok, {A, B, C, D, E, F, G, H}} ->
            {ok, v6,
                (A bsl 112) bor (B bsl 96)
                bor (C bsl 80) bor (D bsl 64)
                bor (E bsl 48) bor (F bsl 32)
                bor (G bsl 16) bor H};
        _ ->
            {error, invalid_ip}
    end.
